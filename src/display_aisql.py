...
def get_playground_input(functionality):
    ...
    if functionality == "Complete":
        if f"prompt_text_{functionality}_data" not in st.session_state:
            st.session_state[f"prompt_text_{functionality}_data"] = get_default_value(
                functionality, "prompt"
            )
        input_data["prompt"] = st.text_area(
            "Enter a prompt:",
            placeholder="Type your prompt here...",
            key=f"prompt_text_{functionality}",
            value=st.session_state[f"prompt_text_{functionality}_data"],
        )
        ...
        if f"query_{functionality}_data" not in st.session_state:
            st.session_state[f"query_{functionality}_data"] = get_default_value(
                functionality, "query"
            )
        ...
        st.session_state[f"text_{functionality}_data"] = input_data["text"]
        input_data["query"] = st.text_input(
            "Enter your query:",
            placeholder="Type your query here...",
            key=f"query_{functionality}",
            value=st.session_state[f"query_{functionality}_data"],
        )
        ...
        if toggle:
            ...
            if f"entities_{functionality}_data" not in st.session_state:
                st.session_state[f"entities_{functionality}_data"] = get_default_value(
                    functionality, "entities"
                )
            input_data["text"] = st.text_input(
                "Enter text for entity sentiment analysis:",
                placeholder="Type your text here...",
                key=f"text_{functionality}",
                value=st.session_state[f"text_{functionality}_data"],
            )
            input_data["entities"] = st.text_input(
                "Enter entities (comma-separated):",
                placeholder="Type your entities here...",
                key=f"entities_{functionality}",
                value=st.session_state[f"entities_{functionality}_data"],
            )
        ...
        if f"categories_{functionality}_data" not in st.session_state:
            st.session_state[f"categories_{functionality}_data"] = get_default_value(
        ...
        st.text_input(
            ...
            placeholder="Type your categories here...",
            key=f"categories_{functionality}",
            value=st.session_state[f"categories_{functionality}_data"],
        )
        ...
        # AI Transcribe uses file inputs instead of text inputs
        # The file selection is handled in the settings section
        pass

    elif functionality == "AI Parse Document":
        # AI Parse Document uses file inputs instead of text inputs
        # The file selection and parsing options are handled in the settings section
        pass
