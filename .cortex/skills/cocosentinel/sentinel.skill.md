...
Execute `cocosentinel/sentinel-pregate` with the artifact content as input:

```
invoke cocosentinel/sentinel-pregate --input <artifact_path>
```

Or pipe the artifact content:

```
invoke cocosentinel/sentinel-pregate
```
...
- Call `cocowisdom/wisdom-writer` with:
  ```json
  {
    "gate": "sentinel",
    "dimension": "G",
...
If outcome is BLOCKED, for each dimension that returned FAIL, call `cocowisdom/wisdom-writer` with:
```json
{
  "gate": "sentinel",
...
- `cocosentinel/sentinel-pregate` runs before any sub-agent is spawned
...
- BLOCKED outcomes written to CocoWisdom via `cocowisdom/wisdom-writer`
