...
- Added thirty-seventh-cycle CocoFlow bootstrap controls: pre-dispatch execution conflict gate, append-only domain surface learning logs, CocoPod liveness validation, and generated `lifecycle/context.json` with prior learnings and policy findings.
- CocoConsole Flow/Quality panels now surface bootstrap context, conflict gate results, CocoPod liveness results, and recent surface learnings.
...
- Added CocoPlus source-parity command contracts for CocoWisdom negative memory and recall: `$wisdom reject`, `$wisdom learn`, `$wisdom learnings`, `$wisdom index`, and `$wisdom recall`.
...
- `templates/cocoplus.toml.template` is now the only canonical packaged config template; the duplicate `.cortex/templates/cocoplus.toml.template` was removed and validation now rejects reintroducing it.
